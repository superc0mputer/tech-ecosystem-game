...
public class EndGameFeedbackManager : MonoBehaviour
{
    ...
    [SerializeField] private TextMeshProUGUI mainResultTitle; // "Victory" or "Game Over"

    [Header("Category Slots")]
    [SerializeField] private FeedbackSlot slotIndustry;
    [SerializeField] private FeedbackSlot slotGovernance;
    [SerializeField] private FeedbackSlot slotCivilSociety;
    [SerializeField] private FeedbackSlot slotInnovation;
    ...
    private void Awake()
    {
        // Auto-Wire if you follow the naming convention
        if (panelEndGame == null) panelEndGame = transform.Find("Panel EndGame").gameObject;
    ...
    public void ShowFeedback(int industry, int gov, int civil, int innov, bool isWin)
    {
        ...
        FillSlot(slotInnovation, "Innovation", innov);
    }
    ...
    private void FillSlot(FeedbackSlot slot, string category, int score)
    {
        ...
        // Optional: Color code Game Over states
        if (score <= 0 || score >= 10) 
        {
            if(slot.stateText) slot.stateText.color = Color.red;
        }
        else
        {
             if(slot.stateText) slot.stateText.color = Color.white; // or green/yellow
        }
    }
